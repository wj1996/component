...
public class Consumer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            connection.setClientID("wj-01"); //如果持久化订阅，此处必须设置一个唯一的ID标识
            connection.start();
            session = connection.createSession(false,Session.AUTO_ACKNOWLEDGE);
            destination = session.createTopic("durableSubscriber");
            consumer = session.createDurableSubscriber((Topic) destination,"wj");  //持久化订阅   解决的就是Topic中的内容正常情况下，一旦发送消息到broker无消费者消费，消息就会被丢失掉
